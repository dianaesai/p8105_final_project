...
### Once implemented, were prevention measures effective in decreasing total excess deaths?
...
mod7 = lm.cluster(excess_death_r ~ length + stay_at_home + mask_mandate + travel_restrictions, data = post_prevent_final, cluster="state")
...
mod7_df=
  ...
  mutate(
    Variables = c("Intercept", "Length of lockdown", "Lockdown order", "Mask mandate", "Travel restrictions")
  ) %>% 
  relocate(Variables) %>% 
  kbl(caption="Linear model output assessing association of prevention measures and excess deaths per 100,000 residents post first peak", digits = 4) %>%
  kable_classic()
